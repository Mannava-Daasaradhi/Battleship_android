...
class BattleViewModel @Inject constructor(
    ...
    sealed class UiEffect {
        ...
        data class NavigateToPassAndPlayGameOver(val gameId: String, val winner: String) : UiEffect()
    ...
    private fun resumePassAndPlayTurn(p1JustFired: Boolean) {
        ...
        if (_uiState.value.gamePhase == GamePhase.GAME_OVER) {
            ...
            viewModelScope.launch {
                _uiEffect.emit(UiEffect.NavigateToPassAndPlayGameOver(gameId, winnerName))
            }
            return
        }
    ...
    private fun handlePlayerShot(coord: Coord) {
        ...
        viewModelScope.launch {
            ...
            if (checkAllSunk(aiPlacements, myShotHistory)) {
                ...
                if (gameMode == GameMode.LOCAL) {
                    val loserName = if (isP1ActiveTurn) p2Name else p1Name
                    _uiState.update { it.copy(gamePhase = GamePhase.GAME_OVER) }
                    _uiEffect.emit(
                        UiEffect.NavigateToPassAndPlayHandOff(
                            gameId = gameId,
                            toPlayerName = loserName,
